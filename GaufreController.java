...
public class GaufreController {
    ...
    private GaufreMediumAI mediumAI;
    private int gamemode; // -1 = no selection, 0 = against AI, 1 = against player
    private int aiLevel; // -1 = no selection, 0 = random AI, 1 = medium AI
    ...
    public GaufreController(GaufreModel model, GaufreView view, GameOverView gameOverView) {
        ...
        this.mediumAI = new GaufreMediumAI();
        this.aiLevel = -1;
            ...
            if (model.isValidMove(y, x)) {
                ...
                if (model.isGameOver() || model.isFull()) {
                    ...
                    if (gamemode == 0 && model.getCurrentPlayer() == 2) {
                        ...
                        switch (aiLevel) {
                            case 0:
                                aiMove = randomAI.getRandomMove(model);
                                break;
                            case 1:
                                aiMove = mediumAI.GetCoupAjoue(model);
                                break;
                            default:
                                aiMove = null;
                        }
    ...
    public static void main(String[] args) {
        ...
        SelectAIView selectAI = new SelectAIView(model,controller);
        ...
        // AI case, we choose the level of the AI
        if (controller.gamemode == 0){
            frame.getContentPane().add(selectAI); // Then add the selectModeView
            frame.pack();
            // Wait for user to make a selection
            while (controller.aiLevel == -1) {
                try {
                    Thread.sleep(100);
                } catch (InterruptedException e) {
                    e.printStackTrace();
            ...
            // Remove the AISelection view
            frame.getContentPane().remove(selectAI);
        }
